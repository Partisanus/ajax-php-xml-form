<?php
function is_guest(){ // функция проверки на гостя пока не работает
...
<?php if(is_guest()):?> <!-- Тут проверка на гостя она еще не закончена -->
    <div id="cl3" style="display:"><!-- блок кнопок которые открывают нужную форму для гостя (авторизации или регистрации) -->
...
<?php else:?> <!-- Если пользователь не гость то выводилась бы строка приветствия с его
                именем пользователь опознается по ID из куков и находится его имя в базе по id -->
    <a> Добро пожаловать  ИМЯ ИЗ КУКОВ</a>
    <br>
